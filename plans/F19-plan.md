...
- `client/api/metadata.ts` —
  ...
  3. **Add `useStoreCascadeInfo(storeId)` query export** — use `queryKey: ['store-cascade', storeId]`; if no `storeId`, return `{ itemPrefsCount: 0, activeListItemsCount: 0 }`; fetch counts in parallel with `Promise.all` using:
     ...
     and return counts with nullish fallback to `0`.
  4. **Invalidate exact query keys on success** — `useUpdateStore` invalidates `['metadata']`; `useDeleteStore` invalidates both `['metadata']` and `['list']`.
...
- `client/api/__tests__/metadata-store-mutations-test.ts` — test new metadata hooks:
  ...
  - `useDeleteStore`: deletes by store id with household filter, throws on null household, invalidates `['metadata']` and `['list']`.
...
- `useDeleteStore` invalidation key is specified as `['list']`, while repository conventions often use `['shopping_list']`; plan follows the spec verbatim.
- `useStoreCascadeInfo` includes an early return for null `storeId` while also setting `enabled: !!storeId`; tests will validate zero-response behavior without forcing network calls when disabled.
