# Implementation Plan: F85 Structured Quantity Data Conversion
...
- `client/lib/parser.ts` —
  1. **Adopt structured package token value** — add file-local `PackageValue { canonical: string; plural: string | null; }`, replace `lookupPackage` usage with `lookupPackageEntry` and classify `PACKAGE` token values as `{ canonical, plural }`.
...
- `client/lib/quantityFormat.ts` —
  ...
  2. **Remove vocabulary dependency in formatter** — drop `DEFAULT_VOCABULARY`/`getPlural` import usage and compute plural in `formatQuantity` as `effectiveCount !== null && effectiveCount > 1 ? (fields.packagePlural ?? `${packageType}s`) : packageType`.
  3. **Export parser utility** — add exported `parseQuantityText(text, vocabulary): QuantityParsed | null` using `tokenize -> classifyTokens -> groupTokens -> assembleCandidate`; return null for empty/whitespace or all-null quantity fields.
...
- `client/app/(tabs)/items.tsx` —
  1. **Load vocabulary for parsing** — import/use `useVocabulary`, `DEFAULT_VOCABULARY`, `parseQuantityText`, `QuantityParsed`; add `const { data: vocabulary } = useVocabulary(); const vocab = vocabulary ?? DEFAULT_VOCABULARY;`.
...
- `client/app/(tabs)/__tests__/items-f85-test.tsx` —
  1. **Create F85 items parsed-payload coverage** — verify create flow sends `default_qty_parsed` for parseable qty and null for blank qty.
  2. **Add alternate parsed alignment tests** — verify `alternate_qtys_parsed` index alignment (`"1 lb, 2 cans"`) and null for unparseable element (`"1 lb, blah"`).
  3. **Add edit and undo parsed restoration tests** — verify edit update payload includes parsed default qty and undo calls `updateItem` with old parsed snapshot.
  4. **Ensure:** test setup follows existing provider/query-client patterns.
...
- `client/app/(tabs)/items.tsx` import list includes `QuantityParsed` in the spec text, but the snippet only uses `parseQuantityText` directly for payload construction. Plan assumes the import should be added only if referenced by local typing needs; otherwise it can remain unused and omitted to avoid lint noise.
