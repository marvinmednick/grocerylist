# Implementation Plan: F104 Per-Entry Store ID (Approved)
...
- `client/app/(tabs)/index.tsx` —
  ...
  7. **Fix `renderItem` secondary text store name** — in `renderItem`, the `secondaryParts` array currently includes `parent.store?.name`; change this to `entry.store?.name`. After F104 `ListItem` no longer has a `store` field, so the old line silently produces nothing.
  8. **Ensure:** Keep screen-level undo registration, multi-user trip flow, modal safe-area/scroll behavior, optimistic drag UI syncing via `localFlatData`, and all non-store editing fields unchanged.
...
- `client/components/SmartAddItem.tsx` —
  ...
  4. **Fix `storeName` prop passed to `DuplicateResolutionDialog`** — the current call site passes `storeName={duplicateMatch?.store?.name}`, which is always `undefined` after F104 since `ListItem` no longer has a `store` field. Change to use the active entry's store name: add a memoized value `const targetEntryForDialog = useMemo(() => duplicateMatch ? getTargetEntry(duplicateMatch) : null, [duplicateMatch])` and pass `storeName={targetEntryForDialog?.store?.name}`.
  5. **Ensure:** Preserve existing duplicate dialog branching, quantity combine/custom logic, mutable tracker redo handling, quick-accept behavior, and all non-store add flows.

- `client/components/DuplicateResolutionDialog.tsx` —
  1. **Fix `existingStoreName` fallback chain** — line 53 currently reads `storeName ?? match?.store?.name ?? 'this store'`. After F104, `match?.store?.name` is always `undefined` since `store` is removed from `ListItem`. Change to `storeName ?? targetEntry?.store?.name ?? 'this store'`. `targetEntry` is already computed immediately below on line 54–63, so move the `targetEntry` memo above this line (or restructure so `existingStoreName` is derived after `targetEntry` is available).
  2. **Ensure:** Leave all dialog layout, button logic, combine/custom/add-new branching, and quantity display logic unchanged.
