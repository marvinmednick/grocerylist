# Implementation Plan: F7 Settings Screen (Approved)
...
- `components/Settings.tsx` — full-screen slide-up modal component with props `{ visible: boolean; onClose: () => void }`; implement sections:
  ...
  2. **Profile section:** call `useHousehold()` to obtain `{ displayName, displayNameShort, avatarColor, householdId }`; initialize `TextInput` for display name pre-filled with `displayName`; initialize `TextInput` for short name pre-filled with `displayNameShort`; initialize `selectedColor` state to `avatarColor ?? PROFILE_COLORS[0].hex`. Render 7-color horizontal picker using `PROFILE_COLORS`; selected circle style `borderWidth: 2`, `borderColor: 'white'`, unselected `borderColor: 'transparent'`; show `"Another member uses this color"` warning (`color: '#991b1b'`) only when selected color exists in `memberColors` from `useHouseholdMemberColors(householdId)`; Save button calls `useUpdateProfile().mutate({ display_name, display_name_short, color })`.
  3. **App section:** Dark Mode row with `Switch` using `value={isDark}` and `onValueChange={toggleTheme}` for immediate persistence.
  4. **Household section:** pass `householdId` (from `useHousehold()`) to `useHouseholdName(householdId)`; render household name as read-only `<Text>` with `ActivityIndicator` while loading.
  5. **Constant:** define exact `PROFILE_COLORS` list at top of file with Blue `#2563eb`, Green `#16a34a`, Orange `#ea580c`, Purple `#9333ea`, Red `#dc2626`, Teal `#0d9488`, Pink `#db2777`.
