...
- `client/components/DuplicateResolutionDialog.tsx` —
  ...
  3. **Restructure the normal-mode options list** — when `showOptions`, render one `<View style={styles.optionsList}>` that contains:
     full-width same-store combine buttons labeled `"Combine as ${option.label}"` and calling `onCombine(option)`;
     cross-store combine buttons for each option labeled `"Combine as ${option.label} in ${existingStoreName}"` and `"Combine as ${option.label} in ${incomingStoreName ?? 'target store'}"` with `onCombine(option, match?.store_id ?? null)` and `onCombine(option, incomingStoreId)`;
  ...
  5. **Remove the combine section heading and retired layout wrappers** — delete the `"Combine as:"` label JSX and stop using the multi-row wrappers that separated combine actions from Add New.
...
- `client/components/__tests__/DuplicateResolutionDialog-test.tsx` —
  1. **Refresh stale assertions for the renamed/repositioned actions** — update the existing tests to assert:
     ...
     cross-store renders the renamed combine labels;
     the bottom row contains `Custom` and `Cancel` but not `Add New`;
     the Add New callback test uses `duplicate-add-separate`;
     and returning from custom mode shows the updated combine label instead of the removed section header.
  ...
  4. **Preserve combine callback coverage** — keep the combine-tap tests verifying `onCombine` receives the same `CombineOption` and store ID behavior as before, but update the queried button text to the new label strings.
...
- The spec is inconsistent on cross-store combine button text. The Files to Modify section says `"Combine as ${option.label} in ${existingStoreName}"` / `"in ${incomingStoreName ?? 'target store'}"`, but Acceptance Criteria and Tests to Write expect `"Combine as 3 lb at Safeway"` / `"Combine as 3 lb at Costco"`. The implementation should resolve whether the final label uses `in` or `at` before work begins.
