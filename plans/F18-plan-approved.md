...
- `client/app/(tabs)/index.tsx` —
  1. **Fetch master item details lazily for List Edit modal (#68)** — add `useItemById(editingItem?.item_id ?? null)` and gate the query so it is only active when the List Edit modal is open and the row is master-linked: `isEditModalVisible && !!editingItem?.item_id`. The hook API itself remains generic (enabled when `itemId` non-null); the dual condition is applied at the call site in the screen.
...
- `useItemById` enabling: spec says hook is enabled when `itemId` is non-null and in `index.tsx` says use only when List Edit modal is open. Implementation applies both conditions together at the call site (`isEditModalVisible && !!editingItem?.item_id`) while keeping the hook API generic (enabled when `itemId` non-null).
