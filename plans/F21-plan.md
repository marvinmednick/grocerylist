# Implementation Plan: F21 Items Screen Enhancements
...
- `client/app/(tabs)/items.tsx` —
  ...
  2. **Add recent-window constants and helpers** — define `RECENT_DAYS = 7` and `RECENT_MS = RECENT_DAYS * 24 * 60 * 60 * 1000`; add `isNewItem(item)` that compares `Date.now()` against `new Date(item.created_at).getTime()`.
