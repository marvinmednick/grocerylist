# Implementation Plan: F17 Item Entry Flow Polish
...
- `client/components/SmartAddItem.tsx` —
  ...
  3. **Add one-off qty chip + popover flow on create row (#59)** — add state:
     ...
     - `"1"` chip active when `oneOffQty === '1'`, sets `'1'`
     - `"Other"` chip active when `oneOffQty !== '1'`, label shows current custom qty, opens popover
...
- `client/components/__tests__/SmartAddItem-test.tsx` —
  ...
  2. **Add #59 coverage (one-off qty chips/popover)** —
     - one-off row shows `"1"` and `"Other"` chips
     - selecting `"Other"` + entering custom qty (e.g. `'3 lbs'`) makes quick-add send that quantity
     - default quick-add still sends `quantity: '1'` when no custom selection is made
