...
- `client/api/profile.ts` —
  1. **Add `id: string` to the `MyProfile` interface** — the query already fetches `*` from `profiles` (which includes `id`), so the field is already in the response; it just needs to be declared in the type.
...
- `client/components/SmartAddItem.tsx` —
  ...
  6. **Obtain current user ID for duplicate classification** — use `myProfile.id` (already available via `useMyProfile()`) and pass it to `classifyDuplicateState(match, incomingStoreId, myProfile.id)`.
  7. **Register undo per spec matrix** — use labels and inverse/redo operations exactly from spec (`Combined {name}`, `Updated {name}`, `Added {name} ({qty})`, `Added {name}`); apply mutable tracker pattern for add-new redo IDs.
  8. **Render and wire `DuplicateResolutionDialog`** — pass match/incoming data, computed combine options, duplicate state, and callbacks; on successful actions call `clearAndClose()`, on dismiss restore saved query.
  9. **Ensure:** Preserve existing non-duplicate add flows when no match is found, maintain quick-accept and edit-before-add behavior, and do not alter unrelated ranking/parser behavior.
...
- `client/lib/duplicateDetection.ts` — add pure `findDuplicate` (master by `item_id`; one-off by case-insensitive trimmed name), `classifyDuplicateState(match, incomingStoreId, currentUserId)` (`active-same-store`, `active-different-store`, `purchased-same-trip`, `purchased-other-user`), and related exported types (`DuplicateMatch`, `DuplicateState`). Uses `currentUserId` to compare against `QuantityEntry.purchased_by` for purchased-state classification.
...
- None — all ambiguities resolved during plan review.

## Review Corrections Applied
1. **`CombineOptions` type location clarified** — `CombineOption` and `CombineOptions` types are defined in `client/lib/quantityFormat.ts` (not in `duplicateDetection.ts`). Removed misleading "`CombineOptions` linkage" from the `duplicateDetection.ts` new file description.
2. **`classifyDuplicateState` signature expanded** — added `currentUserId: string` as third parameter. SmartAddItem obtains the current user ID via `myProfile.id` (from `useMyProfile()`). The `MyProfile` interface in `api/profile.ts` needs `id: string` added (the query already fetches `*` so the field is present in the response). Added `client/api/profile.ts` to Files to Modify.
