...
- `client/api/list.ts` —
  1. **Extend `ListItem` data shape for warnings + short names** — add `warnings?: Array<{ type: string; store_id?: string; comment?: string | null; preferred_stores?: string[]; entered?: string; standard?: string[]; }>` and `master_item?: { short_name: string | null }` to the `ListItem` interface so shopping list rows can render warning badges and compact names.
...
- `client/app/(tabs)/index.tsx` —
  ...
  4. **Apply style updates from spec** — change `itemRow` from `height: 48` to `minHeight: 48` + `paddingVertical: 6`; add/adjust `nameText` (`fontSize: 15`, `fontWeight: '500'`, `color: '#111827'`) and `secondaryText` (`fontSize: 12`, `color: '#6b7280'`, `marginTop: 2`).
...
- `client/components/Toast.tsx` —
  ...
  2. **Apply warning visual and timing behavior** — for `variant === 'warning'`, use container `#fffbeb` background + `1px` border `#fbbf24`, text `#92400e`, and default duration `4000`; keep default variant current styling and `3000` fallback.
...
- `client/components/Settings.tsx` —
  ...
  3. **Add per-type segmented controls with exact option constraints** — `Toast + Badge | Badge | Off` for `avoided`, `unavailable`, `non_standard_qty`; `Badge | Off` only for `non_preferred`; selected segment style `#2563eb` + white text, unselected `#f3f4f6` + `#374151`.
...
- `client/api/items.ts` —
  1. **Add `short_name` to `MasterItem` interface** — include nullable field compatible with DB column type.
...
- `client/api/profile.ts` —
  1. **Export warning preference type** — add and export:
     ...
     ```
  2. **Add `warning_preferences` to profile read data shape** — ensure profile query returns this field (or includes it through existing `*` select typing).
  3. **Add `warning_preferences?: WarningPreferences` to profile update payload** — wire through `useUpdateProfile` mutation and keep profile query invalidation behavior (`['profile']` or existing equivalent key) after updates.
  4. **Ensure:** preserve all existing profile query/mutation behavior unrelated to warning preferences.

- `supabase/full_schema.sql` —
  1. **Reflect new F13 columns in table definitions** — add `items.short_name`, `list_items.warnings`, and `profiles.warning_preferences` to their respective `CREATE TABLE` blocks so full schema matches migrations for new deployments.
...
- `client/components/__tests__/Settings-test.tsx` —
  1. **Add Warnings section coverage** — assert all four warning-type row labels render.
  2. **Add segmented control option coverage** — assert `Toast + Badge`, `Badge`, `Off` for `avoided`.
  3. **Add constrained-option coverage for `non_preferred`** — assert only `Badge` and `Off`.
...
- `client/app/(tabs)/__tests__/items-test.tsx` —
  1. **Add Short Name modal field rendering test** — verify label/input appears in edit modal.
  2. **Add edit-prefill test** — verify modal populates `short_name` from existing item data.
  3. **Add update payload test** — verify save mutation receives `short_name` field.
...
- `client/components/WarningBadge.tsx` — new warning icon/popup component:
  ...
  - shows absolutely positioned popover with type-specific detail strings
  - includes full-screen transparent overlay `Pressable` for outside-tap dismissal
  - uses spec popover style (`right: 0`, `top: '100%'`, white background, radius/shadow/elevation, `minWidth: 200`, `maxWidth: 280`, `zIndex: 1000`)
...
- `WarningBadge` detail strings reference `{store_name}` but warning payload shape only guarantees `store_id`; if no store-name lookup is available in-row, implementation likely needs a deterministic fallback label (for example store id or generic text). Confirm expected fallback text behavior for F13 tests.
