...
- `client/components/Settings.tsx` —
  ...
  6. **UI section insertion** — add a `Quick Accept` section between existing `App` and `Warnings` sections with:
     - `Trigger Word` `TextInput` (`testID="settings-trigger-word-input"`, `autoCapitalize="none"`, `autoCorrect={false}`)
     - invalid helper text `Must be a single word (letters only)` when `!triggerWordValid && triggerWord.length > 0`
     - `Arming Delay (ms)` numeric `TextInput` (`testID="settings-arming-delay-input"`)
...
- `client/components/__tests__/SmartAddItem-quickaccept-test.tsx` — SmartAddItem integration tests with full hook/mutation mocking pattern and fake timers:
  ...
  5. always-on first-row highlight shown (`#eff6ff`)
