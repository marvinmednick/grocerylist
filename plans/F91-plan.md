...
- `client/app/(tabs)/items.tsx` —
  ...
  4. **Active Abbreviations data wiring** — consume `useWordAliases()` and `useWordAliasesForWords()`; build lowercase unique words from `editingItem.name` + current `aliases`; show read-only rows `"{word} → {alias1}, {alias2}"` only for matched words.
  ...
  6. **"Define Abbreviations" launch** — add state `abbreviationsVisible: boolean` and `abbreviationsInitialSearch: string`; render blue text button `Define Abbreviations` below Active Abbreviations that opens `<Abbreviations ... initialSearch={abbreviationsInitialSearch} />` with initial search as item words joined by spaces.
...
- `client/api/items.ts` —
  ...
  3. **Query shape verification/update** — ensure `useAllItems` select shape returns `aliases` explicitly (retain current joins for `category` and `item_store_preferences`); if `*` remains, include explicit `aliases` in select string for clarity.
...
- `client/components/Abbreviations.tsx` — full-screen modal component with:
  ...
  - Placeholder rows for unmatched terms in each mode, including unknown-item-word amber inline warning (`"doesn't appear in any of your items"`).
  - Row tap opens centered-card edit dialog (transparent backdrop, nested pressables).
  - Edit dialog supports existing alias chips with remove `×`, add-alias input, lowercase/trim/single-token validation, suggestion chips from `useAbbreviationSuggestions`, conflict warnings (blocking duplicate alias, amber vocabulary conflict, amber item-name conflict, amber unknown canonical for new entries), Save diff logic (create new/delete removed), and Delete-all-with-confirmation flow for canonical word.
