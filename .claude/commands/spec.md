...
Update the spec file's `GitHub Issue:` line with the returned issue number.
...
**If Review Level is Full — same-session path:**
```bash
...
**If Review Level is Full — new-session path:**
```bash
...
aider --model <model-flag> \
  ...
  [each file from "Files to Modify" and "New Files", one per line]
```
